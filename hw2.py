...
def practice_main2(*rot): #z-buffer 추가
    move_z = 500
    v, f, c = read_obj('Object2.obj')
    obj_bound = v.max(axis = 0) - v.min(axis = 0)
    obj_center = v.min(axis = 0) + obj_bound/2

    imgh, imgw = 300, 300
    scale = 10
    trans = (imgw / 2, imgh / 2, move_z) - obj_center
    v = (v - obj_center) @ rot_matrix(*rot).T * scale + obj_center + trans
    v[:, :2] = v[:, :2] * move_z / v[:, 2, np.newaxis]

    img = np.zeros((imgh, imgw, 3), dtype=np.uint8)
    zbuffer = np.ones((imgh, imgw)) * np.inf
    for face in f:
        tri_verts = v[face - 1, :]

        tri_min = np.ceil(np.min(tri_verts, axis=0))
        tri_max = np.floor(np.max(tri_verts, axis=0)) + 1
        tri_min = np.maximum(tri_min, 0).astype(int)
        tri_max = np.minimum(tri_max, (imgw, imgh, np.inf)).astype(int)
        if np.any(tri_max - tri_min < 0):
            continue
        ...
        if np.any(val):
            ...
            if np.any(testZ):
                ...
                img[y[testZ], x[testZ], :] = C.T[testZ,:] * 255
